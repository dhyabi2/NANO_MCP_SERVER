...
describe('NANO RPC Operations', () => {
    ...
    describe('Send Operations', () => {
        ...
        test('should create send block', async () => {
            ...
            const result = await nano.createSendBlock(TEST_SEND_ACCOUNT, 'test_private_key', // Note: This will fail in production without a real private key
            TEST_RECEIVE_ACCOUNT, TEST_AMOUNT, accountInfo);
            expect(result).toBeDefined();
            expect(result).toHaveProperty('hash');
            expect(result).toHaveProperty('block');
        });
        test('should validate send parameters', async () => {
            await expect(nano.createSendBlock('invalid_account', 'test_private_key', TEST_RECEIVE_ACCOUNT, TEST_AMOUNT, {})).rejects.toThrow();
        });
    });
    describe('Receive Operations', () => {
        ...
        test('should create receive block for pending transaction', async () => {
            ...
            if (Object.keys(pending.blocks).length > 0) {
                ...
                const result = await nano.receiveAllPending(TEST_ACCOUNT, 'test_private_key' // Note: This will fail in production without a real private key
                );
                expect(result).toBeDefined();
        ...
        test('should handle receive for account with no pending blocks', async () => {
            const result = await nano.receiveAllPending('nano_1111111111111111111111111111111111111111111111111111hifc8npp', 'test_private_key');
            expect(Array.isArray(result)).toBe(true);
    ...
    describe('Block Processing', () => {
        test('should create open block for new account', async () => {
            ...
            const result = await nano.createOpenBlock('nano_1newaccount111111111111111111111111111111111111111111111111111', 'test_private_key', sourceBlock, sourceAmount);
            expect(result).toBeDefined();
            expect(result).toHaveProperty('hash');
            expect(result).toHaveProperty('block');
        });
        test('should validate block parameters', async () => {
            await expect(nano.createOpenBlock('invalid_account', 'test_private_key', 'invalid_block', 'invalid_amount')).rejects.toThrow();
        });
    });
